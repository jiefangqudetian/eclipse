			                        ...
			                        <tr  >
			                        	<c:choose>
			                        		<c:when test="${disk.type == 'dir' }">
					                            <td width="50" class="file_icon"><i class="fa fa-folder-o tr" rel="${disk.id }"></i></td>
			                        		</c:when>
			                        		<c:otherwise>
					                            <td width="50" class="file_icon"><i class="fa fa-file-o tr" rel="${disk.id }"></i></td>
			                        		</c:otherwise>
			                        	</c:choose>
			                            <td>${disk.name}</td>
 ...
 <script>
 	$(function(){
 		...
 		$(".rename").click(function(){
 			...
 			layer.prompt({title:"请输入新文件名:"},function(text, index){
 				layer.close(index);
 				$.post("/disk/rename",{"id":id, "name":text}).done(function(resp){
                    if(resp.state == 'success') {
                        layer.msg("修改成功");
                        history.go(0);
                    } else {
